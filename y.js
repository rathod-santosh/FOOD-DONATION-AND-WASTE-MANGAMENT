...
app.post('/accept-delivery/:id', async (req, res) => {
  try {
    ...
    // ✅ Find the existing pending delivery
    ...
    // ✅ Create a new AcceptedDelivery entry
...
app.get('/delivery/accepted', async (req, res) => {
  try {
    const acceptedDeliveries = await AcceptedDelivery.find().lean();
    res.render('delivery/accepted', { acceptedDeliveries });
  } catch (err) {
    console.error("Error fetching accepted deliveries:", err);
    res.status(500).send("Error fetching accepted deliveries.");
...
// app.post("/assign-delivery/:donationId", async (req, res) => {
//   try {
//       const donationId = req.params.donationId;
//       console.log("Received Donation ID:", donationId);
...
//           dropLocation: donation.dropLocation
//       });

//       await newDelivery.save();
//       console.log("✅ New delivery added:", newDelivery);

//       // ✅ Update Donation's `deliveryStatus`
//       await Donation.findByIdAndUpdate(donationId, { deliveryStatus: "pending_delivery" });
